...
// 用于处理重用和内容的显示
@property (strong, nonatomic) AXDCollectionView *collectionView;
// collectionView的布局
...
- (void)commonInit {
    ...
    if ([navi isKindOfClass:[UINavigationController class]]) {
        ...
        if (navi.interactivePopGestureRecognizer) {
            ...
            [_collectionView setupScrollViewShouldBeginPanGestureHandler:^BOOL(AXDCollectionView *collectionView, UIPanGestureRecognizer *panGesture) {
...
- (void)receiveMemoryWarningHander:(NSNotificationCenter *)noti {
    ...
    [_childVcsDic enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull key, UIViewController<AXDScrollPageViewChildVcDelegate> * _Nonnull childVc, BOOL * _Nonnull stop) {
        __strong typeof(self) strongSelf = weakSelf;
        if (strongSelf) {
            if (childVc != strongSelf.currentChildVc) {
                [_childVcsDic removeObjectForKey:key];
                [AXDContentView removeChildVc:childVc];
            }
        }
...
- (void)reload {
    ...
    [self.childVcsDic enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull key, UIViewController<AXDScrollPageViewChildVcDelegate> * _Nonnull childVc, BOOL * _Nonnull stop) {
        [AXDContentView removeChildVc:childVc];
        childVc = nil;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    UICollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:AXDScrollCellID forIndexPath:indexPath];
...
- (void)setupChildVcForCell:(UICollectionViewCell *)cell atIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_isLoadFirstView) { // 第一次加载cell? 不会调用endDisplayCell
        ...
        if (isFirstLoaded) {
            // viewDidLoad
            if ([_currentChildVc respondsToSelector:@selector(viewDidLoadForIndex:)]) {
                [_currentChildVc viewDidLoadForIndex:indexPath.row];
            }
        }
...
- (void)collectionView:(UICollectionView *)collectionView didEndDisplayingCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!self.forbidTouchToAdjustPosition) {
        if (_currentIndex == indexPath.row) {// 没有滚动完成
            ...
            if (_needManageLifeCycle) {
                UIViewController<AXDScrollPageViewChildVcDelegate> *currentVc = [self.childVcsDic valueForKey:[NSString stringWithFormat:@"%ld", (long)_oldIndex]];
                // 开始出现
                [currentVc beginAppearanceTransition:YES animated:NO];
                
                UIViewController<AXDScrollPageViewChildVcDelegate> *oldVc = [self.childVcsDic valueForKey:[NSString stringWithFormat:@"%ld", (long)indexPath.row]];
                // 开始消失
...
- (AXDCollectionView *)collectionView {
    if (_collectionView == nil) {
        ...
        [collectionView registerClass:[UICollectionViewCell class] forCellWithReuseIdentifier:AXDScrollCellID];
        collectionView.bounces = YES;
...
- (NSMutableDictionary<NSString *,UIViewController<AXDScrollPageViewChildVcDelegate> *> *)childVcsDic {
    if (!_childVcsDic) {
