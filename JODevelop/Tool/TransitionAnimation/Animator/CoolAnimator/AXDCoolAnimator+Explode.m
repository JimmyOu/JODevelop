...
@implementation AXDCoolAnimator (Explode)

- (void)setExplodeToAnimation:(id<UIViewControllerContextTransitioning>)transitionContext {
    [self p_animation:transitionContext duration:self.toDuration];
    
}

- (void)setExplodeBackAnimation:(id<UIViewControllerContextTransitioning>)transitionContext {
    [self p_animation:transitionContext duration:self.backDuration];
    
}

- (void)p_animation:(id<UIViewControllerContextTransitioning>)transitionContext duration:(NSTimeInterval)duration{
    ...
    [UIView animateWithDuration:duration animations:^{
        for (UIView *view in snapshots) {
            CGFloat xOffset = [self p_randomFloatBetween:-100.0 and:100.0];
            CGFloat yOffset = [self p_randomFloatBetween:-100.0 and:100.0];
            view.frame = CGRectOffset(view.frame, xOffset, yOffset);
            view.alpha = 0.0;
            view.transform = CGAffineTransformScale(CGAffineTransformMakeRotation([self p_randomFloatBetween:-10.0 and:10.0]), 0.01, 0.01);
        }
...
- (float)p_randomFloatBetween:(float)smallNumber and:(float)bigNumber {
