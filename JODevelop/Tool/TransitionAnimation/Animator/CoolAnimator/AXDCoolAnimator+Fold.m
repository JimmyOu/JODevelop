...
@implementation AXDCoolAnimator (Fold)

- (void)setFoldToAnimation:(id<UIViewControllerContextTransitioning>)transitionContext leftFlag:(BOOL)left {
    [self p_animation:transitionContext flag:left duration:self.toDuration];
    
}

- (void)setFoldBackAnimation:(id<UIViewControllerContextTransitioning>)transitionContext leftFlag:(BOOL)left {
    [self p_animation:transitionContext flag:left duration:self.backDuration];
    
}

- (void)p_animation:(id<UIViewControllerContextTransitioning>)transitionContext flag:(BOOL)flag duration:(NSTimeInterval)duration{
    ...
    for (int i=0 ;i<self.foldCount; i++){
        ...
        UIView *leftFromViewFold = [self p_createSnapshotFromView:fromView location:offset left:YES image:fromImage];
        ...
        UIView *rightFromViewFold = [self p_createSnapshotFromView:fromView location:offset + foldWidth left:NO image:fromImage];
        ...
        UIView *leftToViewFold = [self p_createSnapshotFromView:toView location:offset left:YES image:toImage];
        ...
        UIView *rightToViewFold = [self p_createSnapshotFromView:toView location:offset + foldWidth left:NO image:toImage];
...
- (UIView *)p_createSnapshotFromView:(UIView *)view location:(CGFloat)offset left:(BOOL)left image:(UIImage *)image{
    ...
    UIView* snapshotWithShadowView = [self p_addShadowToView:snapshotView reverse:left];
...
- (UIView *)p_addShadowToView:(UIView*)view reverse:(BOOL)reverse {
