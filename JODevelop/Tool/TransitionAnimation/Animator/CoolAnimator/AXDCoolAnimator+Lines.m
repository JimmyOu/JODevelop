...
@implementation AXDCoolAnimator (Lines)
...
- (void)setLinesToAnimation:(id<UIViewControllerContextTransitioning>)transitionContext vertical:(BOOL)vertical {
    [self p_animateTransition:transitionContext duration:self.toDuration to:YES vertical:vertical];
    
}

- (void)setLinesBackAnimation:(id<UIViewControllerContextTransitioning>)transitionContext vertical:(BOOL)vertical {
    [self p_animateTransition:transitionContext duration:self.toDuration to:NO vertical:vertical];
    
}


#define HLINEHEIGHT 4.0
- (void)p_animateTransition:(id <UIViewControllerContextTransitioning>)transitionContext duration:(NSTimeInterval)duration to:(BOOL)flag vertical:(BOOL)vertical{
    UIViewController *fromVC = [transitionContext viewControllerForKey:UITransitionContextFromViewControllerKey];
    UIViewController *toVC = [transitionContext viewControllerForKey:UITransitionContextToViewControllerKey];
    UIView *toView = toVC.view;
    UIView *containerView = [transitionContext containerView];
    [containerView insertSubview:toView atIndex:0];
    NSArray *outgoingLineViews = [self p_lineViews:fromVC.view intoSlicesOfDis:HLINEHEIGHT Offset:fromVC.view.frame.origin.y containerView:containerView vertical:vertical];
    NSArray *incomingLineViews = [self p_lineViews:toView intoSlicesOfDis:HLINEHEIGHT Offset:toView.frame.origin.y containerView:containerView vertical:vertical];
    CGFloat toViewStart = vertical ? toView.frame.origin.y : toView.frame.origin.x;
    BOOL presenting = flag;
    vertical ? [self p_repositionViewSlices:incomingLineViews moveFirstFrameUp:NO]:[self p_repositionViewSlices:incomingLineViews moveLeft:!presenting];
    ...
    [UIView animateWithDuration:duration - 0.01 delay:0 usingSpringWithDamping:0.8 initialSpringVelocity:0 options:UIViewAnimationOptionCurveEaseIn animations:^{
        vertical ? [self p_repositionViewSlices:outgoingLineViews moveFirstFrameUp:YES] : [self p_repositionViewSlices:outgoingLineViews moveLeft:presenting];
        [self p_resetViewSlices:incomingLineViews toOrigin:toViewStart vertical:vertical];
...
- (NSArray<UIView *> *)p_lineViews:(UIView *)view intoSlicesOfDis:(float)dis Offset:(float)yOffset containerView:(UIView *)containerView vertical:(BOOL)vertical{
    ...
    UIImage *img = [self p_ImageFromsnapshotView:view];
...
-(void)p_repositionViewSlices:(NSArray *)views moveLeft:(BOOL)left{
    CGRect frame;
    float width;
...
-(void)p_repositionViewSlices:(NSArray *)views moveFirstFrameUp:(BOOL)startUp{
...
-(void)p_resetViewSlices:(NSArray *)views toOrigin:(CGFloat)o vertical:(BOOL)vertical{
    CGRect frame;
    for (UIView *line in views) {
        frame = line.frame;
        if (vertical) {
